...
// 配置OLED使用的P32 P33引脚为开漏输出模式
static void OLED_GPIO_Config() {             // OLED引脚配置
    P3_MODE_OUT_OD(GPIO_Pin_2 | GPIO_Pin_3); // I2C通讯引脚
}
...
static void I2C_Config() {
    ...
    I2C_InitStructure.I2C_Mode = I2C_Mode_Master; // 主从模式选择,  I2C_Mode_Master,I2C_Mode_Slave
    I2C_InitStructure.I2C_Enable = ENABLE;        // I2C功能使能,   ENABLE, DISABLE
    I2C_InitStructure.I2C_MS_WDTA = DISABLE;      // 主机使能自动发送,  ENABLE, DISABLE
    I2C_InitStructure.I2C_Speed = 13;             // 总线速度=Fosc/2/(Speed*2+4), 0~63,Fosc是时钟频率
                                                  // 400K=24MHZ/2/(Speed*2+4)  解出Speed=13

    I2C_Init(&I2C_InitStructure);
    NVIC_I2C_Init(I2C_Mode_Master, DISABLE, Priority_0); // 主从模式, I2C_Mode_Master, I2C_Mode_Slave;
    // 中断使能, ENABLE/DISABLE; 优先级(低到高) Priority_0-3
...
static void OLED_Config() {
    ...
    OLED_WR_Byte(0x00, OLED_CMD); //---设置列地址低字节
    OLED_WR_Byte(0x10, OLED_CMD); //---设置列地址高字节
    ...
    OLED_WR_Byte(0xA6, OLED_CMD); //--设置正常显示(非反色)
    OLED_WR_Byte(0xA8, OLED_CMD); //--设置复用比率(1到64)
    OLED_WR_Byte(0x3f, OLED_CMD); //--1/64占空比
    OLED_WR_Byte(0xD3, OLED_CMD); //-设置显示偏移 移位映射RAM计数器(0x00~0x3F)
    ...
    OLED_WR_Byte(0xA4, OLED_CMD); // 禁用全屏点亮(0xa4/0xa5)
    OLED_WR_Byte(0xA6, OLED_CMD); // 禁用反色显示(0xa6/a7)
    OLED_Clear();
    OLED_WR_Byte(0xAF, OLED_CMD); /*显示开启*/
}
void OLED_Init() {
    OLED_GPIO_Config();  // OLED引脚配置
    EAXSFR();            // 拓展寄存器使能打开
    I2C_Config();        // I2C配置
    OLED_Config();       // OLED初始化配置
